...
def post_items_to_erpnext(row):
    ...
    if not name:
        ...
        'allow_alternative_item' : row.get('Allow Alternative Item') or 0,
        'allow_negative_stock': row.get('Allow Negative Stock') or 0,
        'is_purchase_item': row.get('Allow Purchase') or 1,
        'is_sales_item': row.get('Allow Sales') or 1,
        'disabled': row.get('Disabled'),
        'is_stock_item' : row.get('Maintain Stock'),
        'has_variants': int(row.get('Has Variants')) or 0,
        'include_item_in_manufacturing': int(row.get('Include Item In Manufacturing')) or 0,
        'opening_stock': row.get('Opening Stock'),
        'valuation_rate': row.get('Valuation Rate'),
        'standard_rate' : row.get('Standard Selling Rate'),
        'is_fixed_asset': int(row.get('Is Fixed Asset')) or 0,
        'auto_create_assets': int(row.get('Auto Create Assets on Purchase')) or 0,
        'is_grouped_asset': int(row.get('Create Grouped Asset')) or 0,
        ...
        'allow_negative_stock': int(row.get('Allow Negative Stock')) or 0,
        'has_batch_no': int(row.get('Has Batch No')) or 0,
        'create_new_batch': int(row.get('Automatically Create New Batch')) or 0,
        'batch_number_series': row.get('Batch Number Series'),
        'has_expiry_date': int(row.get('Has Expiry Date')) or 0,
        'retain_sample': int(row.get('Retain Sample')) or 0,
        'sample_quantity': row.get('Max Sample Quantity'),
        'has_serial_no': int(row.get('Has Serial No')) or 0,
        'serial_no_series': row.get('Serial Number Series'),
        'variant_of': row.get('Variant Of'),
        'variant_based_on': row.get('Variant Based On'),
        'enable_deferred_expense': row.get('Enable Deferred Expense') or 0,
        ...
        'is_customer_provided_item': int(row.get('Is Customer Provided Item')) or 0,
        'customer': row.get('Customer'),
        'delivered_by_supplier': int(row.get('Delivered by Supplier (Drop Ship)')) or 0,
        ...
        'grant_commission': int(row.get('Grant Commission')) or 0,
        ...
        'published_in_website': row.get('Published in Website') or 0,
